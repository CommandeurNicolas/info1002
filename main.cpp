...
void h2iTest(vector<string> args) {
    ...
    cout << "MD5(\"oups\") = " << hexStr(b,32) << endl << "h2i(MD5(\"oups\"), "<< args[1] << ") = " << myInverter.h2i(b, stoi(args[1])) << endl << endl;
}
...
int main(int argc, char *argv[]) {
    ...
    test->add_option("--i2i", i2iArgs, "compute i2i(i1, t1), i2i(i2, t2), ...")->allow_extra_args(true); // TODO: revoir Q6
    ...
    test->callback( [&](){
        if (isConfig) {
            cout << "CONFIG : \n" << endl;
            cout << "Byte b[32]" << endl << "hashFunction = hash_MD5_string"<< endl << "min_size = 4" << endl << "max_size = 5" << endl
